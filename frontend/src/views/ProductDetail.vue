...
export default {
  ...
  methods: {
    ...
    async addToCart() {
      ...
      if (this.currentUser.userId === this.product.sellerId) {
        showToast('不能添加自己发布的商品到购物车')
        return
      }
      ...
      try {
        ...
        showToast(e.response?.data || '添加失败')
      }
    },
    // ✅ 修改：直接购买跳转到确认订单页面
    async buyNow() {
      ...
      // 构造单个商品的数据结构，与购物车选中的商品格式一致
      const directBuyItem = {
        cartItemId: null,
        productId: this.product.id,
        productTitle: this.product.title,
        productImage: this.product.images && this.product.images.length > 0 ? this.product.images[0] : null,
        price: parseFloat(this.product.price),
        quantity: 1,
        selected: true,
        tradeLocation: this.product.tradeLocation,
        tradeMethod: this.product.tradeMethod,
        conditionLevel: this.product.conditionLevel,
